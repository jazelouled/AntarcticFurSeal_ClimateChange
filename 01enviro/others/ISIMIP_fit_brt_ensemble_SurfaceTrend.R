...
for (q in 1:length(ssp)){
  ...
  product_folder <- paste0(outdir, "/ensemble/SurfaceTrend/", ssp[q])
...
models <-  dir(paste0("C:/Users/Jazel Ouled/Dropbox/PhD/Arctocephalus-gazella/00output/habitat-model-v2/GAZ/brt/males/ensemble/SurfaceTrend/", ssp[k]), recursive = T, full.names = T )
product_folder <- paste0(outdir, "/ensemble/SurfaceTrend/", ssp[k], "/")
full_model_folders <- paste0(outdir, "/ensemble/SurfaceTrend/", ssp[k], "/", "full_model/")
...
if (ssp[k] == "ssp126"){
  ...
  select_model_id <- 22} else if (ssp[k] == "ssp585"){
  select_model_id <- 10}
...
# Save model
saveRDS(mod_full, file = paste0(outdir, "/ensemble/SurfaceTrend/",ssp[k], "/full_model/", sp_code, "_", mod_code, "_", ssp[k],".rds")) 
...
for (j in 1:length(ssp)){
  ...
  full_model_folders <- paste0(outdir, "/ensemble/SurfaceTrend/", ssp[j], "/", "full_model/")
  if (!dir.exists(full_model_folders)) dir.create(full_model_folders, recursive = TRUE)
